...
export const FEATURE_DOC_SECTIONS: FeatureDocSection[] = [
  ...
  },
  {
    ...
    howTo: [
      ...
      "Select fetch / Axios / Python / OkHttp / … and Copy. Paste into the app.",
  ...
  },
  {
    ...
    howTo: [
      "Switch protocol to SSE, pick GET or POST, set the events URL, Connect. Named events, ids, and retry hints show as badges.",
      "Disconnect and Connect again to resume — Pulse sends the last `id` as `Last-Event-ID`.",
      "For WebSocket, use `wss://…` or pick WS, Connect, send text/binary, use Ping — pong frames show in the list.",
      "For GraphQL subscriptions: set body to GraphQL, Connect, wait for connection_ack, Subscribe, then Complete when done.",
    ],
  },
  {
    id: "pre-request",
  ...
  },
  {
    ...
    howTo: [
      ...
      "Toolbar Export → Postman / .env for the selected environment; Import accepts `.env` too.",
    ],
    tips: [
  ...
  },
  {
    ...
    howTo: [
      ...
      "`bun run pulse:cli workspace --workspace python/examples/git-workspace status` (or set `PULSE_WORKSPACE`).",
