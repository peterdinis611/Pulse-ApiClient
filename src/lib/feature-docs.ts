...
export const FEATURE_DOC_SECTIONS: FeatureDocSection[] = [
  ...
  },
  {
    ...
    howTo: [
      ...
      "Explorer transfer menu imports a file; collection ⋯ exports Pulse, Postman, or OpenAPI JSON.",
      "Settings → Data → Collections folder writes the workspace as Git-friendly `*.pulse.json` files.",
    ],
  },
  {
    id: "environments",
  ...
  },
  {
    id: "mcp",
    title: "MCP",
    summary: "Cursor (and other MCP clients) can call the Pulse Rust engine over stdio — send, GraphQL, cURL, collections, bench, OpenAPI, diff.",
    group: "Productivity",
    items: [
    ...
    howTo: [
      ...
      "Built-in prompts cover collection runs, OpenAPI import/export, GraphQL introspection, cURL import, last-run summary, schema validation, and comparing two JSON responses (`pulse_diff`).",
      "OpenAPI conversion writes `python/examples/.out/…json`; read `pulse://last-run` or call `pulse_last_run` after a collection run. Generated files are also `pulse://out/{file}`.",
    ],
  },
  {
    id: "libraries",
...
export const FEATURE_DOC_GROUP_BLURBS: Record<FeatureDocGroup, string> = {
  ...
  Productivity: "Search, shortcuts, libraries, the Python CI satellite, and what stays on this machine.",
};
